...
const categoryMapping = {
  ...
  const Toolbox = () => {
    ...
    const [selectedCategory, setSelectedCategory] = React.useState("frontBumpers");
    ...
    const availableCategories = Object.keys(selectedCar).filter(
      (category) => Array.isArray(selectedCar[category]) && selectedCar[category].length > 0
    );
              ...
              {selectedCategoryData.map((item, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    key={index + 1}
                  >
                    <img src="./img/white-check.png" className={currentSelectedPartIndex === index ? "w-1/2 h-1/2" : "hidden"} />
                  </div>
                </div>
              ))}
            </div>
